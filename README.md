...
**Phase 3 vertical slice 已接通，目前仍是早期 pre-release 開發，正在做真人試玩、平衡與出貨品質補強。** 現有程式包含純 TS sim、Pixi/React UI、Lab→Factory→Shop→Patents 完整循環、1→2→3→4 張圖進程，以及 Save v4 多槽存檔/回溯。新局先在單一 `63×63` 的 Layer A 正中央開始；後續 layer 仍維持 `63×63`，但使用靠近中心、由圖層決定的 phase offset。存讀檔只要求目前 build 內正確，**不承諾跨 build 延續**；更新後可直接清除舊開發存檔。

2026-07 的互動重做已把原本的網站式頁籤／表格／按鈕流程換成全螢幕工廠遊戲 shell：固定資源 HUD、F1–F4 模式 rail、世界畫布、底部 hotbar 與右側 inspector。Factory 支援 LMB 拖建、RMB 拖拆、觸控點放／拖移鏡頭、Shift／MMB 平移、wheel 游標錨定縮放、`R/H/V/Q`、`Ctrl+C/X/V`、50 步 undo/redo；Lab 使用固定 `704×512` 的局部 atlas viewport（40px cell，100% 約看 `17×13` 格），具有逐格 minor grid、每5格 major grid、origin axes、拖曳平移、wheel 游標錨定縮放、`F`／Focus 跟隨藥物，以及 A–D active-layer tabs。地圖療效、副作用、hazard 與 wall 由散點改為連通區域。Lab 另有使用真實多格 footprint、ports 與 belts 的 Pilot Bench；機器可以在 Bench 改變 anchor／footRot並即時重路由，送廠時由唯一 source→analyzer 拓撲重建 Recipe，Factory 逐欄位接收同一份 layout，不再重新 auto-pack。Market／Patents 改為卡片與研究 lattice。完整契約與視覺比較見 [docs/ui-interaction.md](docs/ui-interaction.md)。

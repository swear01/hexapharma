...
- **Recipe 直接操作與放置預覽** ✅：原本右側 raw-text log 已換成底部水平指令軌與共用 SVG machine pictogram。hotbar/數字鍵先拿起機器，插入槽 hover/focus 在 commit 前重算完整 recipe；地圖同步畫 cyan committed route、orange dashed candidate route 與 revealed endpoint ghost。卡片支援 select/scrub、`R/H/Q/Delete/Escape`、pointer-capture drag reorder、50-step undo/redo 與 Run playhead；Run 中鎖住 Recipe authority。preview 與 apply/reveal 共用逐格 sim authority，首次踏入 unknown 就停止公開結果，hidden empty/wall/hazard indistinguishable。

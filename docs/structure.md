...
| `src/ui/` | ✅ | checkpoint同-origin normalized lineage/cross-run replace；compact budgets與共用rewind aggregate。legacy先history preflight、必要時head-alone，mixed legacy回invalid + clean recovery；錯誤可見 |
| `src/main.tsx` | ✅ | React 進入點（掛載 `Root` → `Game`） |
| `test/e2e/` | ✅ | Playwright Chromium對dev`:53347`跑完整smoke/regression、Factory bounded-analysis alert + 固定screenshot baseline；production-preview先build、對`:53348`切四tab驗dynamic chunks/零runtime errors，另載入2-map最寬與4-map最大32×32 authority驗canvas ≤980×980。Playwright預設headless；`*.spec.ts-snapshots/`是expected images，`__screenshots__/`只是artifact |

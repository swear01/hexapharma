...
| `src/sim/drug-graph/` | ✅ | 多圖效果引擎：四特徵 / transform union（translate 四關係 順逆垂直偏移 / scale / swap）/ supercover 掃動（牆停·危險即死）/ fog / 最終位置判定；`previewStep` 暴露與 apply/reveal 共用的 read-only next + 逐格 trail authority |
...
| `src/sim/game.ts` | ✅ | 完整 `GameState` + pure intent reducer；canonical clone/deep-freeze authority inputs；self-declared origin + normalized trace（no-op省略、連續ticks/layout/same-disease sales合併；≤4,096 entries/≤100,000 ticks/≤100,000,000 weighted work）/replay/hash；Game map ≤64 per side/≤4,096 cells、factory ≤256 per side/≤4,096 cells、inventory ≤24,500、bulk ≤100,000；新局`63×63`/1 layer/中心radius-3 fog，專利1→2→3→4且尺寸不變；base `9×6 + patent delta` setFactory authority；每個 factoryTicks cold ownership clone；實體成品/銷售、探索/專利與 core-reachability validator |
...
| `src/render/labRenderer.ts` | ✅ | PixiJS v8 Lab渲染器（只讀sim；一次只畫 A–D active layer，約`11×8`局部視野並cull viewport外 cells；原創 substrate/fog/feature sprites；cyan committed/actual route + orange dashed candidate route/revealed endpoint ghost；unknown由opaque fog遮蔽） |
...
| `src/ui/` | ✅ | viewport game shell、HUD/nav rail/hotbar/inspector/card lattice；Lab 有 `recipeEditor.ts` 純 insert/move/orient/delete/history、`recipePreview.ts` 精確逐格 route/failure/fog mask、`MachineIcon.tsx` 共用語意 SVG，以及底部 held-placement 指令軌／drag reorder／playhead；Factory direct editor與純 camera/grid/gesture/history helpers。visited views保留mount但active guard隔離hotkeys；另含checkpoint lineage/budgets/recovery |

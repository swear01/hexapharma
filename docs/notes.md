...
- **完整/compact都先驗raw work**：full wire ≤5,000,000 chars才round-trip；合法24,500-item state可超wire cap。compact與full `deserializeGame`都從raw origin+intentTrace算work後才replay；single state ≤4,096 entries/≤100,000 ticks/≤100,000,000 work。正常100,000-tick reference約31,000,000、24,500-inventory流程更低。
...
- **Lab 大圖靠實際 dimensions 縮 cell，不改 default snapshot**：preferred cell仍依N為32/26/22px，再按最大實際map width/height與2-column grid縮小，canvas width/height都 ≤980。production-preview同時載2-map最寬與4-map最大32×32，且既有default尺寸 screenshot baseline保持原樣。
- **map patent 是 deeper-level reset，不是 append**：`new-map` / `new-map-4` 會把 nMaps 2→3→4、seed +1、依圖數改尺寸並重生整個目前 level；清 recipe/factory/runtime/waste/inventory/fog **與 `economy.sold`**（reveal-aid 會套到新 fog），保留扣款後 cash/R&D、patents 與全域 inventory ID 序列。Patents UI 必須完整列出破壞範圍並要求 confirmation，不能單擊即清。

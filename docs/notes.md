...
- **Lab 大圖是局部鏡頭，不是縮小全景**：每張地圖預設 `63×63`，Lab canvas 固定 `704×512`，100% 約看 `11×8` 格；drag pan、wheel anchor zoom、`F` follow，A–D tabs 一次只顯示 active layer並保存各自camera。renderer依可見cell bounds cull；production-preview直接載最大`64×64` authority。禁止回退成2-column全圖、動態縮cell或≤980全景。

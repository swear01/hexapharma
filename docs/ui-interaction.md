...
HexaPharma 借鑑三款遊戲不同的強項：Big Pharma 的製藥工廠結構、shapez 2 的低摩擦編輯器、Factorio 的一致工具語言。這是功能與互動原則的綜合，不是任何一款遊戲的外觀 clone。
...
- Lab canvas 與 fog／effect maps 是主畫面；recipe timeline 與 disease readout 位於 inspector。
...
1. **純函式 tests**：camera clamp／anchor zoom、grid hit test、line rasterization、unique cell accumulation、undo／redo branch。
...
3. **Playwright interactions**：viewport shell、HUD／nav／hotbar／inspector 可見；`F1`–`F4`；drag build；RMB drag erase；touch tap／pan；`Q/R/H/V`；clipboard；undo／redo；MMB pan／anchor zoom／camera reset；play／pause；form focus isolation；modal keyboard/focus；390/768/1024 reachability；cards 取代表格。
...
5. **production preview**：lazy renderer、最大 map、不同 viewport 都不得出現 page error、空 canvas、body overflow 或靜默 fallback。

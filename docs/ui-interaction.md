...
![Phase 4 前基線：Lab 已以效果地圖為主工作面，但底部指令軌仍以插入槽／card reorder 作 authority；D19 將由 Pilot Bench 取代。](assets/ui-study/after-lab.png)
...
## 4. HexaPharma Phase 4 UI 契約

本節是本輪要收斂到的 active contract。2026-07-12 已完成 40px Atlas 格線、連通 feature 視覺、真實多格機器、Pilot Bench anchor 移動／footRot／自動重路由、唯一 source→sink 拓撲驗證，以及 exact layout 送廠；目前底部 Recipe track 仍是過渡編輯入口。hotbar 直接落 Bench、Bench undo/erase/ghost、Atlas↔Bench 雙向高亮、可拖 divider 與唯讀 derived timeline 仍列在 [plan.md](plan.md) 的未完成 Phase 4 工作，不能把本節目標誤報成已全部交付。後續 UI 修改若改變已落地行為或目標契約，必須同步更新本文與對應測試。
...
- Lab 的數字鍵／pointer 把實體機器或 belt 拿到 Pilot Bench cursor，先顯示和 Factory 同源的 footprint、ports、方向與合法／非法 ghost，再由 pointer 對 Bench commit；Effect Atlas 同步顯示 fog-safe route ghost。Factory 的 `1`–`6` 分別選 Belt、Splitter、Merger、Source、Sink、Erase。
...
Lab 是兩個同步空間，不是 Recipe 表單加一張結果圖：

- **Effect Atlas** 是 `63×63` 效果地圖的主要探索面。viewport intrinsic size 固定 `704×512`，100% cell 約 `40px`、可見約 `17×13` 格；禁止為了同時看完全圖或多圖而縮成全景。
- Atlas 每格都有低對比 minor grid，每 5 格有較強 major grid，origin 軸與座標刻度另有層級。grid 畫在 substrate／fog 上方、route／feature／token 下方；遠縮放可淡出 minor grid，但 major grid 必須保留。格線在 unknown 中可見，feature／區域輪廓／potency 不可見。
- pointer drag 平移，wheel 以游標錨定縮放（75–225%）；`F` 或 `Focus` 將鏡頭對準藥物並進入 follow。手動 pan／zoom 會離開 follow，camera 只屬 UI state。解鎖多層後以 A–D tabs／hotkey 切 active layer；一次只畫一層，各層保留 camera 並 cull viewport 外 cells。
- 新局 fog visibility radius 3，先揭露 `7×7 = 49/3969` 格；未知區以 opaque microscope fog 完全遮蔽，不顯示「?」或牆／危險／療效輪廓。revealed 療效／副作用／危險／牆壁以連通 fill + border 呈現，療效區另有疾病 label；若 sim 資料提供 potency core，再以第二層輪廓顯示，不由 renderer 自行猜強度。

**Pilot Bench** 是可執行的實體實驗產線：

- 使用與 Factory 共用的約 `40px` 地板格、machine shapes、ports、belt、collision、rotation、ghost/buildability 與 gesture semantics；不得維護一套只供 Lab 的近似 footprint 規則。大 footprint 靠 pan/zoom 與清楚輪廓閱讀，不靠把每格放大回網頁卡片尺寸。
- 初期只接受唯一 source、唯一 analyzer/sink、無 cycle、無 splitter/merger 的單一 source→sink 路徑。未連通、多出口、分支、cycle、缺 machine port 或占地衝突都在焦點附近顯式報錯，不能猜一條 Recipe 順序。
- machine hotbar 使用與 Bench footprint、timeline、inspector 相同的語意 pictogram。拿起機器後 ghost 同時顯示完整占地、input/output ports、footRot、effect orientation、預計 Atlas segment 與合法性；`R` 預設同步旋轉 footprint + effect，`V` 可明確只改 effect orientation，`H` 改 effect flip，`Q` pipette，RMB／Erase 拆除，Escape 清 cursor。
- 每個合法 pointer gesture 只形成一筆 Bench layout history；`Ctrl/Cmd+Z/Y` undo/redo。hover／選取 Bench machine 會在 Atlas 高亮相對應 route segment；從 timeline scrub 某一步則反向定位並高亮 Bench machine。
- desktop 預設 Atlas 主、Bench 次，兩者以可拖曳 divider 分配約 65/35 空間；`Tab` 交換主焦點但不改 authority。compact 允許 Atlas／Bench focus toggle，非焦點空間仍保留 live overview、當前 machine 與 warning，不得退化成兩個不相干頁面。

**Recipe timeline 是衍生 readout**：

- 系統由 Pilot Bench 唯一 source→sink connectivity 確定性推導 `Template.steps`。timeline 只顯示 breadcrumb、step outcome、processing cost/speed、錯誤與 Run playhead；不得提供 insert slot、card reorder 或另一份獨立 undo history。
- Recipe preview 與 sim 共用 `previewStep` 的逐格 sweep authority，牆停、hazard、scale、Phase Exchange 斷線不得由 UI 重寫近似邏輯。一旦進入第一個 unknown cell，只畫已知 prefix 並標示 uncertain；hidden empty／wall／hazard 必須產生相同預覽。
- Run 時 timeline playhead 跟隨由拓撲推導的 machine order，Atlas 畫實際 cyan route，Bench 同步 pulse 當前機器；Space 執行／停止實驗。Clear 是清空實驗原型的低頻可復原動作。
- Phase Exchange A↔B 在只有 A 時是明示 locked state；B 解鎖後才可選。inspector 顯示 before/after 語意，不以「交換0/1層」內部編號當玩家說明。
- `Send prototype to Factory` 先原子驗證 Bench 拓撲、Template outcome、Factory entitlement 與完整 ProductionBlueprint。成功後 Factory 接收逐欄位相同的 layout／anchors／rotations／tiles／routing；任何失敗顯式拒絕，禁止 auto-pack 或 silent repair。
...
- Factory editor 與 Pilot Bench editor history 各最多保留 50 個完整 mutation snapshot；clipboard、selection、held machine、hover 與 history 是 UI editor state。committed ProductionBlueprint 是 layout authority，`Template.steps` 只能由其唯一 source→sink topology 推導後進 Run／Game authority。
...
- Atlas 編輯時以 cyan 顯示已連通 blueprint route、橙色虛線顯示 held／hover machine 的候選 segment，橙色半透明 capsule 是 revealed 候選終點；Bench 同時以相同 selection color 高亮對應 footprint 與 ports。Run 動畫畫實際 cyan history 並 pulse 當前實體機器。兩者都使用逐格 sweep，Phase Exchange 是不掃格的瞬移且前後斷線。unknown route/endpoint 被 fog mask；token halo 只標目前位置。Clear prototype 清 blueprint/route，探索 fog 仍持久保留。
...
1. **純函式 tests**：Lab/Factory camera、40px adaptive major/minor grid、Pilot Bench placement/rotate/delete/history、唯一 source→sink topology validation 與 Template derivation、逐格 preview/failed step/swap break/fog mask、SVG icon semantics、Factory grid/gesture/history。
...
3. **Playwright interactions**：viewport shell、HUD／nav／hotbar／inspector 可見；`F1`–`F4`；Lab Atlas grid/camera/layer、Bench exact placement／collision／routing／history、雙向 machine highlight、timeline scrub/playhead、invalid topology rejection、exact Factory transfer、Phase Exchange lock與fog 49/3969；Factory direct manipulation；form isolation、modal focus、390/768/1024 reachability。
4. **visual regression**：固定 viewport 比對 Atlas+Bench fogged、revealed connected regions、Factory exact transferred prototype，以及 Market／Patents 現行 baseline；評估 canvas 占比、遮擋、層級與 overflow，不以和競品的 pixel similarity 作成功標準。文件另保存 redesign 前後對照，但競品圖片不進 baseline。
5. **production preview**：lazy renderer、最大`64×64` Game map、固定`704×512` Atlas viewport/culling、不同 browser viewport 都不得出現 page error、空 canvas、body overflow 或靜默 fallback。

關鍵資料驗收：由 Bench 推導的 timeline 不可編輯；invalid／ambiguous topology 不可產生猜測模板；Lab→Factory 前後 ProductionBlueprint 的 layout 尺寸、tiles、anchors、footRot、effect orientation／flip、ports 與 routing 必須逐欄位相等。
...
- Lab 使用同步 Atlas + Pilot Bench、Factory 使用 production floor；兩者共用 canvas placement language、hotbar 與 inspector。Market／Patents 使用 cards；modal 只保護高影響重置。

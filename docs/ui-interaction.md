...
![After：Lab 以效果地圖為主工作面，配合頂部 transport、底部 hotbar 與右側 recipe inspector。](assets/ui-study/after-lab.png)
...
- Lab 的數字鍵把機器加入 recipe timeline；Factory 的 `1`–`6` 分別選 Belt、Splitter、Merger、Source、Sink、Erase。
...
- Lab canvas 與 fog／effect maps 是主畫面；recipe timeline 與 disease readout 位於 inspector。預設只有一張 `63×63` Layer A，start/origin 在正中央 `(31,31)`。
...
- machine hotbar 以數字鍵或 pointer 加入步驟。
- `R` 設定下一台 orientable machine 的 rotation，`H` 切 effect flip。
- Backspace／Delete 移除最後一步，Space 執行／停止實驗。
...
- Run、Clear、Remove last、Send recipe to Factory 仍提供 visible controls，但快捷與 hotbar 是同一 state 的另一入口，不是第二套邏輯。

### 4.7 Inspector

- desktop 的 Lab／Factory inspector 固定在世界右側；compact viewport 移到世界下方。兩者內容皆可獨立捲動，世界本身不因長資訊上下移動。
...
- editor history 最多保留 50 個完整 gesture snapshot；clipboard 與 history 都是 UI editor state，不進 sim core，也不繞過 `setFactory` authority。
...
- Lab Run 動畫以 sim 的逐步 DrugState 在 active layer 畫出 cyan route history；Phase Exchange 是不掃格的瞬移，交換前後必須斷線，不能畫成穿牆實線。token halo 只標目前位置，不能再把單格 halo 誤稱為完整 path。Reset／編輯 recipe 會清 route，探索 fog 仍持久保留。
...
1. **純函式 tests**：Lab/Factory camera clamp／anchor zoom、Lab focus/visible-cell bounds、grid hit test、line rasterization、unique cell accumulation、undo／redo branch。
...
3. **Playwright interactions**：viewport shell、HUD／nav／hotbar／inspector 可見；`F1`–`F4`；Lab drag/wheel/`F` follow、A–D layer camera、單層Phase Exchange lock與fog 49/3969；Factory drag build、RMB drag erase、touch tap／pan、`Q/R/H/V`、clipboard、undo／redo、MMB pan／anchor zoom／camera reset；play／pause；form focus isolation；modal keyboard/focus；390/768/1024 reachability；cards 取代表格。

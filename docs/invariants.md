...
- **routing state/方向**：splitter 只收 `inDir`，成功離開時依該 tile 的 round-robin cursor 從 `outDirs` 選路；merger 只收 `inDirs`，同 tick contender 按 `inDirs` 陳列順序固定優先。per-splitter cursor 影響未來行為，必須進 runtime、cold snapshot/restore、hash、Save v3 且範圍合法。
- **空間與 authority bounds**：public factory-sim area ≤65,536；Game factory 另限每邊 ≤256、總計 ≤4,096 格。machines ≤area、每 shape 的 cells/inPorts/outPorts 各 ≤256、aggregate shape cells ≤area、aggregate input/output ports 各 ≤262,144；active belt unit 必須在界內、位於 carrier tile且每格最多一顆，每台機器最多持有一顆。restore 即使在 production 也不得靠 TypedArray 靜默截斷非法值。
...
- **完整 intent replay**：同初始state+intents→逐欄位/hash相同；trace ≤4,096、factory ticks ≤100,000、weighted work ≤100,000,000。正常100,000-tick reference約31,000,000，24,500-inventory流程更低。no-op省略，連續ticks/layout/same-disease sales正規化；`replayTicks`恰等raw trace總和。

...
HexaPharma 目前是早期開發中的 pre-release 專案。正確性閘、確定性與目前 build 內的 Save v10 存讀檔仍須可靠，但**不承諾任何跨 build 的存檔相容性**。
...
- 舊 Blueprint／Save schema 由當前 reader 顯式拒絕；Blueprint v3、Save v9或其舊versioned storage key不得被當成v4／v10 payload解析，拒絕也不得覆寫原blob。測試中的 `legacy` 是 rejection與storage完整性測試，不代表支援任意歷史 build。

## 仍然必須守住

- 同 build、同完整 `GenOptions`、同 seed 與同 input trace 必須逐欄位重現。
- 當前 build 的合法Save v10必須round-trip；stepwise Research／DiscoveredFormula、paid Production build trace、non-null Production與cold runtime都必須重播一致。corrupt／partial／偽造authority必須顯式拒絕，不可冒充成功。

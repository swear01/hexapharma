...
1. **Research**
   ...
   - assay 只顯示目標的 local／八方向寬廣 sector，不公開座標、距離或 reference。
...
- Blueprint v3 codec 仍識別只存 ordered `{typeId}` 的 `research-program`，但 UI 對它只提供import／download／delete，不提供capture／apply；`factory-layout` 存 routing 與 `{id,typeId,anchor,footRot}`，可正常建立與套用。
- Factory Blueprint 不綁來源場域，可免費開到 Production Plan，或付費 Commission 到 Production。Library 與 save slots 分離並跨存檔。
- Save v9 保存 non-null Production layout/runtime、paid build intents、stepwise Research／formulas、內部 pilot、economy 與 fog；舊開發版顯式拒絕。
...
- Atlas 與 Factory 都仍是正方格，但 geometry／validator 分離；本版不做 true-hex migration。

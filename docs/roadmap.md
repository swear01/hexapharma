...
### Phase 1 — 最小可見（看得到謎題）
- PixiJS 平面並列畫 2 張正方格地圖（四特徵 + 迷霧）+ 機器擺放（旋轉/flip）+ 藥物雙圖同步掃動；React 研究室 UI；Playwright smoke + 固定 `toHaveScreenshot` baseline
...
- **strict factory runtime / routing / authority bounds** ✅：fixed SoA runtime綁layout + `MultiMap` identity，routing cursor進snapshot/hash/save，成功tick零配置。public mapgen/factory各≤65,536 cells；Game map≤32/side/≤1,024、factory≤256/side/≤4,096；base entitlement`9×6 + patent delta`。`sideEffectId`為Int32；public patent invalid state與effect aggregate overflow顯式reject。
...
- **核心修正** ✅：補回 offset（第四種關係）+ supercover 對角掃動；求解器複合難度；mapgen 跨圖張力；Lab 玩生成關卡。
- **Phase 1 — 最小可見（:53346）** ✅：PixiJS v8 並列畫多圖、React 研究室 UI、雙圖同步掃動、Playwright headless smoke + 固定 screenshot baseline；手動可解一條雙圖藥方達 WIN。

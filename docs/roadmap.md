...
完成 Market、Knowledge、shipping contracts、patents、intent replay/checkpoint。當前 wire 是 Save v9，早期 schema 不再支援。
...
- 新局只揭露中心 5×5；assay只透露寬廣方向sector，每個stamp立即resolve，結果同時顯示已知 Cure 與 SideEffect。
...
- Blueprint v3 codec仍驗證`research-program`，但UI不capture/apply它；generic `factory-layout`可正常建立／套用。Save v9保存stepwise Research/formulas、non-null Production與paid build trace。
- Orbital Wet-Lab Schematic使用vector-only嚴格俯視2D，移除generated lab bitmap／runtime manifest contract；Atlas與Factory仍是分離的正方格authority，本階段不做hex migration。

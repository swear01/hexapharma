...
// tick, then every unit (id, pos, each map position, proc, failed flag), then the
// produced drugs (positions + failed), then deadlocked. Iteration follows the
// state's own (deterministic, id-sorted) ordering, so two equal runs hash equal.
